...
target_size = (30, 30)                    # Targeted scaling size
...
# Bulid the net
model = genDeepres(pic_classes, (30, 30, 3))
# plot_model(model, 'deepres.png', show_layer_names=True, show_shapes=True)

# Compile and save model
# sgd = SGD(lr=0.0003, momentum=0.9, decay=1e-5, nesterov=True)
adam = Adam(lr=0.001)
model.compile(optimizer=adam, loss='categorical_crossentropy', metrics=['accuracy'])
save_model(model, filepath='../models/deepres.hdf5')

# Training
# model = load_model('../models/deepres.hdf5')
model.fit_generator(
    train_generator,
    ...
    callbacks=[
        ModelCheckpoint('../models/deepres_weights.hdf5', verbose=1, save_best_only=True, save_weights_only=True),
        TensorBoard('../logs/deepres/')
    ]
)