...
# Bulid the net
model = genResvgg(pic_classes, (32, 32, 3))
# plot_model(model, 'resvgg.png', show_layer_names=True, show_shapes=True)

# Compile and save model
# sgd = SGD(lr=0.0003, momentum=0.9, decay=1e-5, nesterov=True)
adam = Adam(lr=0.001)
# model = load_model('../models/resvgg.hdf5')
model.compile(optimizer=adam, loss='categorical_crossentropy', metrics=['accuracy'])
model.load_weights('../models/resvgg_weights.hdf5', by_name=True)
save_model(model, filepath='../models/resvgg.hdf5')
...
model.fit_generator(
    ...
    callbacks=[
        ModelCheckpoint('../models/resvgg_weights.hdf5', verbose=1, save_best_only=True, save_weights_only=True),
        TensorBoard('../logs/resvgg/')
    ]
)